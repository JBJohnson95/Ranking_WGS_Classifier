...
for (k in 1:length(input)){
  ...
  setwd(dir = "/Users/James/Desktop/Dissertation/Final_Figures/Normalized_Tables/IBD/")  
  
  myT1=read.table(file=paste('Kraken2_IBD_',input[k],'_Normalized.csv',sep=""),header=TRUE,sep = ",")
  myT2=read.table(file=paste('Metaphlan2_IBD_',input[k],'_Normalized.csv',sep=""),header=TRUE,sep = ",")
  ...
  Mean_myT1=log10(colMeans(10^myT1[,4:ncol(myT1)]-1)+1)
  Mean_myT2=log10(colMeans(10^myT2[,4:ncol(myT2)]-1)+1)
  
  #Kruskal Wallis test for Kraken2
  pValuesDisease_myT1=vector()
  Name_myT1 <- vector()
  
  for(i in 4:(ncol(myT1))){
    Disease=factor(myT1$IBD)
    kruskal <- kruskal.test(formula=myT1[,i]~Disease)
    pValuesDisease_myT1[i-3] <- kruskal$p.value
    Name_myT1[i-3] <- names(myT1)[i]
  }
  FDR_Corrected_pValuesDisease_myT1=p.adjust(pValuesDisease_myT1, method = "BH")
  
  #Kruskal Wallis test for Metaphlan2
  pValuesDisease_myT2=vector()
  Name_myT2 <- vector()
  
  for(i in 4:(ncol(myT2))){
    Disease=factor(myT2$IBD)
    kruskal <- kruskal.test(formula=myT2[,i]~Disease)
    pValuesDisease_myT2[i-3] <- kruskal$p.value
    Name_myT2[i-3] <- names(myT2)[i]
  }
  FDR_Corrected_pValuesDisease_myT2=p.adjust(pValuesDisease_myT2, method = "BH")
  ...
  for (x in 1:length(taxa_names)){
    if ((taxa_names[x] %in% myT1_colnames)&&(taxa_names[x] %in% myT2_colnames)){
      ...
      Kraken2_Adjusted_Pvalue[x]=FDR_Corrected_pValuesDisease_myT1[Name_myT1==taxa_names[x]]
      Metaphlan2_Adjusted_Pvalue[x]=FDR_Corrected_pValuesDisease_myT2[Name_myT2==taxa_names[x]]
    }
  ...
  pdf(paste("Kruskal_Wallis_IBD_Metaphlan2_vs_Kraken2_Average_Means_",input[k],".pdf",sep=""))
  plot(Kraken_means,Metaphlan_means,xlim =c(0,8),ylim=c(0,8),xlab = "Log Normalized Kraken2",ylab="Log Normalized Metaphlan2",
       main = paste("IBD Metaphlan2 vs Kraken2 Averge Value at ",input[k]," level\nR2 is ",R2,sep=""),
