...
for (k in 1:length(input)){
  ...
  FDR_Corrected_pValuesSampleType_Normalized_myT2=p.adjust(pValuesSampleType_Qiime, method = "BH")
  ...
  pdf(paste("Vanderbilt_16S_Normalized_RDP_Highest_Spearman_",input[k],".pdf",sep=""))
  plot(ordered_Normalized_Mean_myT1, maxCors_1,xlab = "Log Mean",
       ylab="Rho",main=paste("Vanderbilt RDP Spearman at ",input[k],sep=""),ylim=c(-1,1),xlim=c(0,6.5),
       col= ifelse(ordered_FDR_Normalized_myT1[-c(1)]>0.05,"black","orange"),pch=16)
  legend("bottom",inset=c(0,.01),c("Insignificant","Significant for RDP"), 
         pch = c(16, 16),cex=0.8,col=c("black","orange") )
  dev.off()
