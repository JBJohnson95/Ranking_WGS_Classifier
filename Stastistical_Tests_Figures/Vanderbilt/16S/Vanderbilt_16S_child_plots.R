...
for (k in 1:length(input)){
  ...
  pdf(paste("New_Vanderbilt_RDP_Spearman_",input[k],".pdf",sep=""))
  plot(log10(ordered_Mean_myT1[2:length(ordered_Mean_myT1)]),highest,xlab = "Log Mean",
       ylab="Rho",main=paste("Vanderbilt RDP Spearman test at ",input[k]," level",sep=""),ylim=c(-1,1),
       col= ifelse(ordered_FDR_Normalized_myT1[-c(1)]>0.05,"black","orange"),pch=16)
  legend("bottom",inset=c(0,.05),c("Insignificant","Significant for RDP"), pch = c(16, 16),cex=0.8,
         col=c("black","orange") )
  dev.off()
  
  
  pdf(paste("New_Vanderbilt_Qiime_Spearman_",input[k],".pdf",sep=""))
  plot(log10(ordered_Mean_myT2[2:length(ordered_Mean_myT2)]),highest_2,xlab = "Log Mean",
       ylab="Rho",main=paste("Vanderbilt Qiime Spearman test at ",input[k]," level",sep=""),ylim=c(-1,1),
       col= ifelse(ordered_FDR_Normalized_myT2[-c(1)]>0.05,"black","green"),pch=16)
  legend("bottom",inset=c(0,.05),c("Insignificant","Significant for Qiime"),
         pch = c(16, 16),cex=0.8,col=c("black","green") )
  dev.off()
