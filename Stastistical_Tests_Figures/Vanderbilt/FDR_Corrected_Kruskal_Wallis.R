...
for (k in 1:length(input)){
  ...
  myT1=read.table(file=paste('Kraken2_Vanderbilt_Forward_Only_',input[k],'_Normalized.csv',sep=""),header=TRUE,sep = ",")
  myT2=read.table(file=paste('Metaphlan2_Vanderbilt_Forward_Only_',input[k],'_Normalized.csv',sep=""),header=TRUE,sep = ",")
  ...
  overlap_names=intersect(colnames(myT1[,6:ncol(myT1)]),colnames(myT2[,6:ncol(myT2)]))
  
  myT1_colnames=colnames(myT1[,6:ncol(myT1)])
  myT2_colnames=colnames(myT2[,6:ncol(myT2)])
  
  Mean_myT1=log10(colMeans(10^myT1[,6:ncol(myT1)]-1)+1)
  Mean_myT2=log10(colMeans(10^myT2[,6:ncol(myT2)]-1)+1)
  
  #Kruskal Wallis test for Kraken2
  pValuesSampleType_myT1=vector()
  Name_myT1 <- vector()
  
  for(i in 6:(ncol(myT1))){
    SampleType=factor(myT1$sample_type)
    kruskal <- kruskal.test(formula=myT1[,i]~SampleType)
    pValuesSampleType_myT1[i-5] <- kruskal$p.value
    Name_myT1[i-5] <- names(myT1)[i]
  }
  FDR_Corrected_pValuesSampleType_myT1=p.adjust(pValuesSampleType_myT1, method = "BH")
  
  #Kruskal Wallis test for Metaphlan2
  pValuesSampleType_myT2=vector()
  Name_myT2 <- vector()
  
  for(i in 6:(ncol(myT2))){
    SampleType=factor(myT2$sample_type)
    kruskal <- kruskal.test(formula=myT2[,i]~SampleType)
    pValuesSampleType_myT2[i-5] <- kruskal$p.value
    Name_myT2[i-5] <- names(myT2)[i]
  }
  FDR_Corrected_pValuesSampleType_myT2=p.adjust(pValuesSampleType_myT2, method = "BH")
  ...
  for (x in 1:length(taxa_names)){
    if ((taxa_names[x] %in% myT1_colnames)&&(taxa_names[x] %in% myT2_colnames)){
      ...
      Kraken2_Adjusted_Pvalue[x]=FDR_Corrected_pValuesSampleType_myT1[Name_myT1==taxa_names[x]]
      Metaphlan2_Adjusted_Pvalue[x]=FDR_Corrected_pValuesSampleType_myT2[Name_myT2==taxa_names[x]]
    }
  ...
  pdf(paste("Kruskal_Wallis_Vanderbilt_Metaphlan2_vs_Kraken2_Average_Means_",input[k],".pdf",sep=""))
  plot(Kraken_means,Metaphlan_means,xlim =c(0,8),ylim=c(0,8),xlab = "Log Normalized Kraken2",ylab="Log Normalized Metaphlan2",
       main = paste("Vanderbilt Metaphlan2 vs Kraken2 Averge Value at ",input[k]," level\nR2 is ",R2,sep=""),
